...
The dataset was split into a training, validation, and testing set based on a 60/20/20 rule.  Several algorithms were used to build separate models based on the training data: random forest, stochastic gradient boosting, CART, and linear discriminant analysis.  The effectiveness of each model was estimated using the validation data.  These four models were then combined using the validation data to create one final model using a random forest algorithm.  Finally, the accuracy was evaluated using the testing set.

## Results

The results of each of the four models with the validation set is given below:
...
It can be seen that the top performing model is clearly the random forecast model, with an accuracy of over 99%, compared to the next highest stochastic gradient boosting accuracy of 96%.  The CART and LDA models performed poorly in comparison, with accuracies of 48% and 69%, respectively.
...
Unsurprisingly, this gives nearly the exact same results as the random forest model, with an accuracy of 99.1%.  the 95% confidence interval for the out-of-sample accuracy is [98.9%, 99.4%], with an expected out of sample error of ~1%.
